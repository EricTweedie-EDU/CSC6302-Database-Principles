...
BEGIN
    ...
    FROM Teacher AS t
    JOIN Student AS s ON t.badge_id = s.teacher_id
    WHERE t.teacher_first_name = v_teacher_first_name AND t.teacher_last_name = v_teacher_last_name;
END $$
delimiter ;

-- procedure for showing all students with teacher and class
-- Modify user procedure
DROP PROCEDURE IF EXISTS showAllStudents;
delimiter $$
CREATE PROCEDURE showAllStudents()
BEGIN
	SELECT s.first_name AS First, s.last_name AS Last, s.email_address AS Email,
		CONCAT(t.teacher_first_name, ' ', t.teacher_last_name) AS Teacher,
		c.subject AS Course, s.class_grade AS Grade
    FROM Student AS s
    JOIN Teacher AS t on s.teacher_id = t.badge_id
